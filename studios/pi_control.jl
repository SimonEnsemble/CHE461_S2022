...
🐢 the smart and busy operator PI-controller settings pertain only to first order systems without a sensor. however, here the sensor is fast compared to the process. apply the smart and busy operator PI-controller settings to this closed loop to determine $K_c$ and $\tau_I$, while ignoring the dynamics of the sensor.
"
...
🐢 numerically invert each transfer function, `Y★`, `Y_sp★`, `E`, `U★`, `U★_P`, and `U★_I`, into the time domain using `simulate` and assign the output to a data frame. e.g. `data_y★ = simulate(Y★, final_time)`. we will plot the data next.
"

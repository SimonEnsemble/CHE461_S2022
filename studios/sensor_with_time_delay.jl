...
💡 a key feature is that the sensor exhibits a time delay of $\theta$. the goal of this studio is to study the influence of this time delay---and the PI controller settings---on the closed-loop response.
...
🐯 apply the smart and busy operator (SBO) PI controller tuning rule to design a PI-control law $G_c(s)$ and assign it as a variable `G_c`. 

!!! note
	to apply the SBO PI-controller tuning rules, set the integral time constant $\tau_I$ of the controller as the time constant of the process _plus_ the time delay of the sensor. the idea is to add the time scales of the process and sensor which _together_ comprise the open-loop transfer function to which SBO PI-controller rules pertain. this rule effectively reduces I-action when there is an additional time delay in the sensor.
...
# ╔═╡ adc54b04-00ca-4f29-bade-c69000889a48
...
use (i) the open-loop transfer function `G_OL` you defined previously and (ii) the `ClosedLoopTransferFunction` constructor [here](https://simonensemble.github.io/Controlz.jl/dev/controls/#feedback-loops-with-time-delays)---necessary to handle the time delay.
...
!!! hint
	you'll need to adjust your PI control law if you hard-coded $\theta=1$ instead of using the variable `θ` I provided.
...
🐯 (this may help) does the peak deviation of $y$ from its set point correspond to the peak in the error signal? why not?
...
manifest_format = "2.0"
project_hash = "a5b84b12f20a44959af44308282215ed71c777bf"

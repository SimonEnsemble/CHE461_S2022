...
💡 the direct substitution method aims to find the ultimate (threshold) controller gain $K_{c,u}$ (u = \"ultimate\") that makes the closed loop marginally stable.

!!! note
	marginal stability is at the boundary of stability and instability---the impulse response of a marginally stable system will present undamped oscillations.

❓ why is knowing $K_{c,u}$ important?
* we wish to avoid choosing (too aggressive) controller settings that make the closed loop unstable.
* some controller tuning methods, e.g. the Ziegler–Nichols tuning method, use (a) the ultimate gain and (b) the period of the oscillations at marginal stability to set the parameters of a PID controller.
...
1. the ultimate controller gain, $K_{c,u}$, that makes the closed loop marginally stable.
2. the frequency $\omega_u$ and period $P_u$ of oscillations in the closed-loop response when the controller gain is set $K_c:=K_{c,u}$.

define them as variables `K_cu`, `ω_u`, and `P_u` below.
"

# ╔═╡ 9961c3d1-7192-4c88-a863-f94572b3ef56
# ultimate controller gain K_cu

# ╔═╡ 53e41cbf-8af9-497b-956f-af8d9d1f7a3a
# ultimate frequency ω_u
...
🦖 suppose the gain of the P-controller is set at the ultimate gain, $K_c=K_{c,u}$. construct the controller transfer function `G_c`.
"
...
md"🦖 use `characteristic_polynomial` (see [here](https://simonensemble.github.io/Controlz.jl/dev/tfs/#Controlz.characteristic_polynomial)) to construct the characteristic polynomial of the closed loop when $K_c:=K_{c,u}$. assign it as a variable `char_poly`. does it agree with the one you derived on pencil and paper?"

# ╔═╡ 8dc8e3cb-9923-41a1-8902-df61bd9af160

# ╔═╡ 2c051e80-7aa8-4088-afe5-41bc257f8f56
md"🦖 use `roots` to compute the roots of the characteristic polynomial of the closed loop when $K_c:=K_{c,u}$. do the roots agree with your expectations? precisely, how so?
...
🦖 first, derive the closed-loop transfer function $Y^*(s)/D^*(s)$ giving the closed-loop response to disturbances (when the set point is fixed) and assign it as a variable `G_reg` (reg = \"regulator\").

🦖 then, use `viz_poles_and_zeros` (see [here](https://simonensemble.github.io/Controlz.jl/dev/viz/#poles-and-zeros-of-a-transfer-function)) to visualize the poles and zeros of the closed-loop transfer function `G_reg`. do some poles of `G_reg` lie in a particular location in the complex plane $\mathbb{C}$ that you'd expect given $K_c:=K_{c, u}$?
...
🦖 simulate the closed-loop response to a unit impulse in the disturbance variable (with $K_c:=K_{c,u}$).
* construct `D★`
* construct `Y★`
...
🦖 does the response conform to your expectations, given $K_c=K_{c, u}$? precisely, how so? mention $P_u$ or $w_u$ you calculated above as well.
...
now that we've determined the ultimate controller gain $K_{c,u}$ and associated oscillation period $P_u$, the Ziegler–Nichols (ZN) PID controller tuning method (see [here](https://en.wikipedia.org/wiki/Ziegler%E2%80%93Nichols_method)) provides a simple formula for how to set the parameters of a PID-controller for disturbance rejection. this is a widely used heuristic controller tuning method. (see [here](https://en.wikipedia.org/wiki/Heuristic) for the definition of \"heuristic\".)

❗ hopefully, this illustrates the very practical value of analyzing the stability of closed-loop control systems. methods for how to determine optimal PID controller parameters rely on the ultimate gain $K_{c,u}$ and associated oscillation period $P_u$!
...
manifest_format = "2.0"
project_hash = "8aad7e0295d8b79bba1824f9780037daf2745f77"
...
git-tree-sha1 = "dea5923171d3c3c2bee4654516f5f056ffb3ab3d"

...
a level controller (LC) receives a measurement of the liquid level from a level transmitter (LT) and, via a control valve, adjusts the flow rate $u$ [m$^3$/min] of a liquid feed stream to maintain $h$ at a set point $h_{sp}$ [m]. the disturbance here is an additional liquid feed stream into the tank that flows at a variable and unpredictable rate $d$ [m$^3$/min].
"

# ╔═╡ b7ed51a0-d254-47ab-8f10-6606ad95f3bd
html"<img src=\"https://raw.githubusercontent.com/SimonEnsemble/CHE461_S2022/main/studios/sim_p_control/liquid_level_control.png\" width=350>"
...
```math
\begin{equation}
    A\frac{dh}{dt}=u + d - h /R
\end{equation}
```
with:
*  $A$ [m$^2$]: cross-sectional area of tank from helicopter view
*  $R$ [min / m$^2$]: resistance to flow in exit line

the values of these parameters are defined below, in addition to `h_max`, which is the height of the tank. if the liquid level rises above `h_max`, the tank overflows."

# ╔═╡ c7113d5a-3e62-4a8d-bd3f-b8bf219ac3e8
begin
	h_max = 6.0  # tank height, m
	A = 4.0      # tank cross-sectional area from helicopter view, m²
...
* we wish to maintain $h$ at $h_{sp}=4$ [m]. thus, the normal, steady state value for $h$ is $\bar{h}=4$ [m].
"
...
md"a block diagram showing the simplified feedback control loop is below. (let's simplify the controller so it takes in the error signal $e$ and outputs $u$, thereby neglecting the current-to-pressure transducer, sensor, and valve dynamics.)
"

# ╔═╡ 1d8c89f4-c17b-46c5-8b92-880ee2eccef8
html"<img src=\"https://raw.githubusercontent.com/SimonEnsemble/CHE461_S2022/main/studios/sim_p_control/block_diagram.png\" width=600>"

# ╔═╡ 70f0aeca-ed91-4e7c-91fe-9faaec8df94e
md"
✏ derive the transfer functions $G_u(s)$ and $G_d(s)$.

🐌 define the transfer functions $G_u(s)$ and $G_d(s)$ using `Controlz.jl` as variables `G_u` and `G_d`, respectively. see [docs on defining transfer functions](https://simonensemble.github.io/Controlz.jl/dev/tfs/)."
...
🐌 define the Laplace transform of $d$, $D(s):=\mathcal{L}[d(t)]$ as a variable `D` below.
"
...
md"🐌 we will run our simulations of the response for $t\in[0, 45]$ min. define `t_final` below."
...
as a baseline to judge the quality of our feedback control system, let's first consider the *open-loop* response *without* an active level controller that adjusts $u$. 

i.e., let's simulate the open-loop response $h(t)$ to the step input in $d$ when $u$ is maintained at its steady state value $\bar{u}$, i.e. when no controller action is taken to counteract the disturbance.

🐌 define `H_no_control` as the Laplace transform of the step response $\mathcal{L}[h^*(t)]=H^*(s)$ when the controller is \"off\". 
...
md"🐌 simulate the response to the step in the disturbance variable (without control) using `simulate` in `Controlz.jl`. assign the data table containing the output as a variable `data_no_control`.
...
now consider the *closed-loop* response when a level controller in the form of a P-controller with controller gain $|K_c|=2.0$ is active. let's simulate the closed-loop response $h(t)$ to the step input in the disturbance $d(t)$ when the P-controller is actively counteracting this disturbance.
...
md"✏ use your block diagram algebra skills to find $G_r(s):= H^*(s)/D(s)$, the transfer function that gives the closed-loop response $h^*(t)$ to changes in the disturbance $d(t)$ (while the set point is constant). the notation \"r\" in $G_r(s)$ indicates this transfer function describes the closed-loop **r**egulator response. 

🐌 define the transfer function $G_r(s)$ as a variable `G_r` below. this will allow us to simulate the closed-loop response.

!!! hint
	you can just multiply and divide the transfer functions `G_u`, `G_c`, and `G_d` appropriately to define `G_r`.
"
...
md"🐌 define `H_w_control` as the closed-loop response $H^*(s)$ to the step in the disturbance $d(t)$, this time with the P-conroller \"on\" (hence \"closed-loop\")."

# ╔═╡ 7950943f-20cd-4b98-aa8b-fa4d0f7e7816


# ╔═╡ e817237b-8aaa-4d90-bde2-28b9bb100364
md"🐌 finally, simulate the closed-loop response $h^*(t)$ using `simulate`. assign the data table containing the output as a variable `data_w_control`."

# ╔═╡ c3292620-b13e-48bc-a2cf-c39b5ec5f3b8


# ╔═╡ 4d522f60-9ff9-4b7f-bcd1-b2b44c76df09
md"🐌 on the same panel, plot:
* a curve giving the open-loop response $h(t)$ (not in deviation form)
* a curve giving the closed-loop response $h(t)$
* a horizontal, dashed line giving $h_{sp}$
* a horizontal, dotted line giving $h_{max}$

draw a legend labeling each curve as \"controller off\", \"controller on\", and \"set point\". include x- and y-axis labels.

!!! hint
	Makie docs [here](https://makie.juliaplots.org/stable/).
"
...
md"🐌 what is the value of the steady state offset error? calculate it from the output of `simulate`."
...
Controlz = "e034abe6-471a-4d54-96dd-ecd1f4022419"
...
manifest_format = "2.0"
project_hash = "fd448da287f9f43bf08b52f2cf5a4766d29febce"
...
[[deps.Controlz]]
deps = ["CairoMakie", "ColorSchemes", "Colors", "DataFrames", "DifferentialEquations", "Interpolations", "Polynomials", "Printf", "Roots"]
git-tree-sha1 = "e76e496d9a79b65439371bf3b7eced6a08cecec7"
